...
def simulate(world, num_steps, policy):
    ...
    for step in range(num_steps):
        ...
        for name, agent in world.agents.items():
            ...
            current_state = agent.position
            agent.move(action, world) if action in ['north', 'south', 'east', 'west'] else None
            agent.pickup(world) if action == 'pickup' else None
            agent.dropoff(world) if action == 'dropoff' else None
            next_state = agent.position
            reward = -1 if action in ['north', 'south', 'east', 'west'] else 0
            reward = 13 if action in ['pickup', 'dropoff'] else reward
