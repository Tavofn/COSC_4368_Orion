...
class RL:
    ...
    customActionsForDebugging = [Action(ActionType.WEST), Action(ActionType.WEST), Action(ActionType.SOUTH),
                                 Action(ActionType.SOUTH), Action(ActionType.PICKUP), Action(ActionType.SOUTH)]
    ...
    def qLearn(self):
        ...
        for x in range(0, self.steps):  #self.steps
            ...
            self.agent.interface.updateAgentPosition(self.agent)
            ...
            oldAgentPos.qValue = (1 - self.alpha) * oldAgentPos.qValue + self.alpha * (
                        self.action.reward + self.discount_factor * self.maxFutureReward(newAgentPos))  # update q
            self.agent.interface.updateQTable(oldAgentPos.position[0], oldAgentPos.position[1], self.action.type.value,
                                              round(oldAgentPos.qValue, 3))
